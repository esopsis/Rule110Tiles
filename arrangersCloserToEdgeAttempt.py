...
def generateRow(canvas, activeRow, arrangeIndex, isLeftLoop, isLeftPrimeLoop,
    ...
    if 0 < halfWidthsFromLeft(canvas, activeRow[0]) <= 2:
        ...
        if 0 < halfWidthsFromRight(canvas, activeRow[-1]) <= 2: 
            end -= 1
            #print("in")
        ...
        if (0 < halfWidthsFromRight(canvas, activeRow[-1]) <= 2 and not
              overrideDirection == canvas.LEFT):
            start = 0
            # print("in2")
    #foo
    ...
    for i in range(start, end):
        ...
        if (0 < halfWidthsFromLeft(canvas, activeRow[0]) <= 2 or
                0 < halfWidthsFromRight(canvas, activeRow[-1]) <= 2):
...
def setColor(canvas, newRow, tileIndex, activeRow, selectedTiles, selectedTile,
    ...
    if topSide is None:
        ...
        if (tileIndex == 0 and 0 < halfWidthsFromLeft(canvas, activeRow[0]) <=
                2 or overrideDirection == canvas.LEFT):
            topBin = objectToBin(activeRow[-1])
        else:
            topBin = 0
    else:
    ...
    for i in range(len(newRow)):
        ...
        if centerObject is not None and centerObject.isAddedRowNewTile:
            ...
            isConflict, selectedTile, selectedTiles = setColor(canvas, newRow,
                    ...
                    isLeftPrimeLoop, isLeftDoublePrimeLoop, isRightLoop,
                    isRightDoublePrimeLoop, overrideDirection, grid, gridRes,
                    isArranging)
            if newRow[i] is None:
                #fooo
                print("in", newRow, i)
...
def doArranging(canvas, grid, gridRes, groupGrid, arrangeIndex, mouseLoc,
    ...
    if isArranging and now - canvas.lastTick >= objects.Canvas.ARRANGE_TIME:
        ...
        if (1 <= halfWidthsFromLeft(canvas, activeRow[0]) <= 2 or
                1 < halfWidthsFromRight(canvas, activeRow[-1]) < 2):
